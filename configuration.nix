...
let
  user      = "admin";
  password  = "password";
  hostname  = "69214898d25654ffdf3a13ad";
  homeDir   = "/home/${user}";
  ...
  wifiModeSwitch = pkgs.writeShellApplication {
    ...
    text = ''
      ...
      if nmcli connection up robot-wifi; then
        # STA is up; keep AP disabled to avoid channel conflicts.
        nmcli connection modify robot-ap connection.autoconnect no || true
        echo "[wifi-mode] STA connected; leaving AP down" >&2
      else
        echo "[wifi-mode] STA connection failed; re-enabling AP" >&2
        nmcli connection modify robot-ap connection.autoconnect yes || true
        ensure_ap
      fi
    '';
